...
class _DeviceScreenState extends State<DeviceScreen> {
  ...
  void initState() {
    ...
    _connectionStateSubscription = widget.device.connectionState.listen((state) async {
      ...
      if (state == BluetoothConnectionState.connected) {
        _services = []; // must rediscover service
      }
  ...
  Future onDiscoverServicesPressed() async {
    ...
    try {
      ...
      configuration = await _services[2].characteristics[3].read();
  ...
  List<Widget> _buildServiceTiles(BuildContext context, BluetoothDevice d) {
    ...
    if(_servicesDiscovered) {
      List<String> result = ["A", "B", "C", "D"];

      print(_services);
      //sleep(const Duration(seconds: 30));
      print("Configuration: $configuration");
      result = configuration.map((number) => String.fromCharCode(number))
          .join('')
          .split(";");
      print(result);
          ...
          .map(
            ...
            ServiceTile(
                ...
                characteristicTiles: s.characteristics.map((c) =>
                    _buildCharacteristicTile(
                        c, result[s.characteristics.indexOf(c)])).toList(),
                names: result
            ),
      )
          .toList();
  ...
  CharacteristicTile _buildCharacteristicTile(BluetoothCharacteristic c, name) {
